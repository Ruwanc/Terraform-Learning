terraform {
  ...
  backend "s3" {
    ...
    key    = "production/data-storage/mysql/terraform.tfstate"  #Make sure to change this key value to match the current file path to get 1:1 mapping between state file and file path
    region = "ap-south-1"
...
module "mysql_rds_instance_module" {
  ...
  mysql_db_name = "production-mysql-db"
  mysql_rds_instance_type = "db.t3.micro"
  # mysql_rds_db_username = "admin"
  # mysql_rds_db_password = "password1234"
